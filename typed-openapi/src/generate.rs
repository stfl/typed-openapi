...
impl Settings {
    ...
    pub fn write_to(&self, crate_dir: impl AsRef<Path>) -> Result<Vec<PathBuf>, GenerateError> {
        ...
        write_bytes(
            &spec,
            self.corrected_header().as_bytes(),
            corrected.yaml.as_bytes(),
        )?;
        write_rust(
            &types,
    ...
    fn corrected_header(&self) -> String {
        format!(
...
/// Write `header` followed by `body`, creating the directory if it is missing.
fn write_bytes(path: &Path, header: &[u8], body: &[u8]) -> Result<(), GenerateError> {
