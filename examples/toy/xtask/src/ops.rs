...
//! document's own names, and hands the result to the one request builder the
//! CLI also uses. That is why nothing here restates what `types.rs` already
//! says.
//!
...
use typed_openapi::{Document, Operation};
...
struct Emitted {
    ...
    id: String,
    /// The subcommand name the CLI mounts it under.
    command: String,
...
pub(crate) fn emit(api: &OpenAPI, model: &Document) -> Result<String> {
...
fn gather(api: &OpenAPI, model: &Document) -> Result<Vec<Emitted>> {
    model
        .iter()
        .map(|op| {
            let (path_item, operation) = find(api, op)
                .with_context(|| format!("`{}` is not in the overlaid document", op.id()))?;
            ...
            Ok(Emitted {
                ...
                method: wrapper(api, op, path_item, operation)?,
            })
        })
        .collect()
}
...
///
/// `None` covers an operation with no body and one whose body this CLI sends
/// verbatim — neither has a type to hold a `--json-body` file to.
fn json_body_type(op: &Operation, operation: &openapiv3::Operation) -> Result<Option<TokenStream>> {
    match op.body() {
...
fn wrapper(
    ...
    let Signature {
        ...
        builder,
        notes,
    } = signature_of(api, op, item, operation)?;
...
fn signature_of(
    ...
    for param in op.params() {
        ...
        let ty = param_type(api, item, operation, param.name())?;
        let wire = param.name();
        if param.required() {
...
fn param_type(
    ...
    let declared = item
        ...
        .with_context(|| format!("`{name}` is not declared on this operation"))?;
    let openapiv3::ParameterSchemaOrContent::Schema(schema) = &declared.parameter_data_ref().format
    else {
        bail!("`{name}` is declared with `content`, not `schema`");
    };
    scalar_type(api, schema)
}
...
fn scalar_type(_api: &OpenAPI, schema: &ReferenceOr<Schema>) -> Result<TokenStream> {
    if let Some(name) = ref_name(schema) {
        let ident = format_ident!("{name}");
        return Ok(quote!(crate::types::#ident));
    }
    let ReferenceOr::Item(schema) = schema else {
        bail!("only `#/components/schemas/` references are followed");
    };
    let SchemaKind::Type(kind) = &schema.schema_kind else {
        bail!("only `type:` schemas have a scalar spelling");
    };
    Ok(match kind {
        ...
        Type::Object(_) | Type::Array(_) => bail!("not a scalar"),
    })
}
...
fn body_type(operation: &openapiv3::Operation) -> Result<TokenStream> {
    let Some(ReferenceOr::Item(body)) = &operation.request_body else {
        bail!("requestBody $refs are not followed");
    };
    let Some(media) = body
        .content
        .iter()
        .find_map(|(name, media)| typed_openapi::schema::is_json(name).then_some(media))
    else {
        bail!("no JSON request body");
    };
